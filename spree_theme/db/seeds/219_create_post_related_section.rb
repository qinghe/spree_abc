...
logo = Spree::Section.create_section(section_piece_hash['container'].id, {:title=>"Post title"},
  {'block'=>{'disabled_ha_ids'=>'111'}, 'inner'=>{'15hidden'=>bool_true}})
logo.add_section_piece(section_piece_hash['container-link'].id).add_section_piece(section_piece_hash['post-title'].id)

logo = Spree::Section.create_section(section_piece_hash['container'].id, {:title=>"Post time"},
  {'block'=>{'disabled_ha_ids'=>'111'}, 'inner'=>{'15hidden'=>bool_true}})
logo.add_section_piece(section_piece_hash['post-time'].id)

logo = Spree::Section.create_section(section_piece_hash['container'].id, {:title=>"Post author"},
  {'block'=>{'disabled_ha_ids'=>'111'}, 'inner'=>{'15hidden'=>bool_true}})
logo.add_section_piece(section_piece_hash['post-author'].id)

logo = Spree::Section.create_section(section_piece_hash['container'].id, {:title=>"Post body"},
  {'block'=>{'disabled_ha_ids'=>'111'}, 'inner'=>{'15hidden'=>bool_true}})
logo.add_section_piece(section_piece_hash['post-body'].id)
