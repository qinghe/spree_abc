...
#end
Spree::Section.where(:title=>'option values in slide style').each(&:destroy)

section_piece_hash= Spree::SectionPiece.all.inject({}){|h,sp| h[sp.slug] = sp; h}

logo = Spree::Section.create_section(section_piece_hash['container'].id, {:title=>"option values in slide style"},
  {'block'=>{'disabled_ha_ids'=>'111'}, 'inner'=>{'15hidden'=>bool_true}})

logo.add_section_piece(section_piece_hash['option-values-in-slide-style'].id)