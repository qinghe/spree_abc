...
order_address = Spree::Section.create_section(section_piece_hash['container'].id, {:title=>"password recover form"},
  {'block'=>{'disabled_ha_ids'=>'111'}, 'inner'=>{'15hidden'=>bool_true}})
order_address.add_section_piece(section_piece_hash['container-title'].id)
order_address.add_section_piece(section_piece_hash['password-recover-form'].id)
