module Spree
  class SectionParam < ActiveRecord::Base
    ...
    begin 'these methods only for development and system improvement'
      ...
      # * params
      #   * html_attribute_key -  html_attribute, html_attribute.id or id+'unset'
      def add_default_value( html_attribute_key, html_attribute_value )
        # key requires string, json required
        html_attribute_key = case html_attribute_key
          when Spree::HtmlAttribute
            html_attribute_key.id.to_s
          when String
            html_attribute_key
          else
            html_attribute_key.to_s
          end

        self.default_value[html_attribute_key] = html_attribute_value
        self.save!
        #get related param_values, if html_attribute_id is nil, set default value
        self.param_values.each{|pv|
          if pv.pvalue[html_attribute_key].nil?
            pv.pvalue[html_attribute_key] = html_attribute_value
