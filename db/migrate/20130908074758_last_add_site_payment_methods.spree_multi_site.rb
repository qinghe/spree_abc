...
class LastAddSitePaymentMethods < ActiveRecord::Migration
  ...
  def up
    ...
    table_name = ActiveRecord::Base.connection.table_exists?(:configurations) ? :configurations : :spree_configurations
    add_column table_name, :site_id, :integer
    ...
    if ActiveRecord::Base.connection.table_exists?(:spree_pages)
