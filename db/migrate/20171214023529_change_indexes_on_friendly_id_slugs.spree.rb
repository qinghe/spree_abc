...
class ChangeIndexesOnFriendlyIdSlugs < ActiveRecord::Migration[5.0]
  def change
    ...
    add_index :friendly_id_slugs, [:slug, :sluggable_type], length: { name: 100, slug: 30, sluggable_type: 20 }
    add_index :friendly_id_slugs, [:slug, :sluggable_type, :scope], length: { name: 100, slug: 30, sluggable_type: 20, scope: 10 }, unique: true
  end
end
